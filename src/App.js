import React, { Component } from "react";

import Weather from "./container/Weather/Weather";
import Card from "./components/Card/Card";
import Wrapper from "./container/Wrapper/Wrapper";
import Spinner from "./components/Spinner/Spinner";

import search from "./assets/svg/search.svg";

import axios from "axios";
import "./App.css";

class App extends Component {
    state = {
        city: null,
        showWeather: true,
        weather: {},
        statusCode: null,
    };

    cityChangeHandler = event => {
        const city = event.target.value;
        this.setState({ city: city });
    };

    keyPressEvent = event => {
        if (event.key === "Enter") this.getWeather();
    };

    getWeather = () => {
        this.setState({ showWeather: false });
        axios
            .get("https://weather.ameykhaire.tech/api/v1/weather", {
                params: {
                    city: this.state.city,
                    units: "metric",
                },
            })
            .then(resp => {
                const data = resp.data.data;
                this.setState({
                    weather: {
                        date: data.date,
                        icon: data.icon,
                        temp: data.temp,
                    },
                    showWeather: true,
                    statusCode: resp.status,
                });
            })
            .catch(err => {
                this.setState({ showWeather: false });
                Promise.reject(err);
            });
    };

    render() {
        let weather = <Card title='Waiting for input...' icon={search} />;

        if (this.state.showWeather && this.state.statusCode === 200) {
            weather = (
                <Weather
                    date={this.state.weather.date}
                    icon={this.state.weather.icon}
                    temp={this.state.weather.temp}
                />
            );
        }
        if (!this.state.showWeather) {
            weather = <Spinner />;
        }
        return (
            <Wrapper>
                <div className='title'>
                    <h3>The Weather App</h3>
                </div>
                <div className='fields'>
                    <div className='input'>
                        <input
                            type='text'
                            placeholder='Enter city..'
                            onChange={event => this.cityChangeHandler(event)}
                            onKeyPress={event => this.keyPressEvent(event)}
                        />
                    </div>
                    <button className='btn' onClick={this.getWeather}>
                        <svg fill='#001' viewBox='0 0 20 20'>
                            <path d='M18.125,15.804l-4.038-4.037c0.675-1.079,1.012-2.308,1.01-3.534C15.089,4.62,12.199,1.75,8.584,1.75C4.815,1.75,1.982,4.726,2,8.286c0.021,3.577,2.908,6.549,6.578,6.549c1.241,0,2.417-0.347,3.44-0.985l4.032,4.026c0.167,0.166,0.43,0.166,0.596,0l1.479-1.478C18.292,16.234,18.292,15.968,18.125,15.804 M8.578,13.99c-3.198,0-5.716-2.593-5.733-5.71c-0.017-3.084,2.438-5.686,5.74-5.686c3.197,0,5.625,2.493,5.64,5.624C14.242,11.548,11.621,13.99,8.578,13.99 M16.349,16.981l-3.637-3.635c0.131-0.11,0.721-0.695,0.876-0.884l3.642,3.639L16.349,16.981z'></path>
                        </svg>
                    </button>
                </div>
                {weather}
            </Wrapper>
        );
    }
}

export default App;
